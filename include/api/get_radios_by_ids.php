...
foreach ($idArray as $id) {
    if(strpos($id, '527782') !== false) {
        ...
        $myRadios[] = '{"id":'.$radioRow['xid'].',"kind":"radio","program_name":"","radio_name":"'.addslashes($radioRow['title']).'","radio_desc":"'.addslashes($radioRow['description']).'","schedule_id":0,"support_bitrates":[64],"rate24_aac_url":"",
			"rate64_aac_url":"'.$radioRow['streamurl'].'",
			"rate24_ts_url":"","rate64_ts_url":"","radio_play_count":1,"cover_url_small":"http://'.$domain.'/images/radiologos/thumb/thumb_'.$radioRow['logo'].'","cover_url_large":"http://'.$domain.'/images/radiologos/'.$radioRow['logo'].'","updated_at":0,"created_at":0}';
    } else {
